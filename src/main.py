...
async def main():
    ...
    # 生成摘要并发送钉钉
    summary_lines = []
    summary_lines.append(f"### 竞彩数据采集完成")
    summary_lines.append(f"- 采集时间：{now_str()}")
    summary_lines.append(f"- 比赛数量：{len(result_array)}")
    high = sum(1 for m in result_array if m['自检']['数据覆盖等级'] == '高')
    mid = sum(1 for m in result_array if m['自检']['数据覆盖等级'] == '中')
    low = sum(1 for m in result_array if m['自检']['数据覆盖等级'] == '低')
